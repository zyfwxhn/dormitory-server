...
public interface LostFoundMapper {
     ...
     */
    @Insert("INSERT INTO lost_found (student_id, type, title, description, category, location, contact_info, images, status) " +
            "VALUES (#{studentId}, #{type}, #{title}, #{description}, #{category}, #{location}, #{contactInfo}, #{images}, #{status})")
     ...
     * 根据主键查询单条信息
     */
    @Select("SELECT lf.*, s.name AS studentName, s.student_no AS studentNo FROM lost_found lf LEFT JOIN student s ON lf.student_id = s.id WHERE lf.id = #{id}")
